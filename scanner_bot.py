"""
scanner_bot.py - Djmere Scanner Bot (WITH KISS confirmation filter - webhook mode)
Compatible with Python 3.14 + python-telegram-bot 21.x

Changes vs the original polling version:
  1. Runs via WEBHOOK instead of polling -> fixes
     "telegram.error.Conflict: terminated by other getUpdates request"
     which happens when two instances briefly overlap during a Render deploy.
  2. get_top_movers() now fetches TOP GAINERS and TOP LOSERS SEPARATELY and
     merges them, so LONG setups (big drops) get equal representation next
     to SHORT setups (big pumps), instead of depending on one combined
     ranking that tends to skew toward pumps for low-cap alts.
  3. Optional KISS confirmation filter (4H Range -> Sweep -> Change of State),
     computed directly from real OHLC data via fractal pivot detection.
     Toggle live with /kiss on|off. The existing Claude-based strategy is
     unchanged; KISS only filters which results get sent.

IMPORTANT (Render setup): this must be deployed as a "Web Service" (not a
Background Worker) so it has a public URL and a $PORT to listen on. Render
sets RENDER_EXTERNAL_URL automatically for web services - no manual config
needed for the webhook URL itself.
"""
...
# Render sets this automatically for Web Services, e.g. "https://myapp.onrender.com"
...
#
# Uses real fractal pivot detection (swing highs/lows), not a fixed lookback
# window, and a proper CHoCH definition: after a liquidity sweep of a swing
# point, structure only "changes" when price closes beyond the most recent
# OPPOSITE swing point that formed during that move.
# ---------------------------------------------------------------------------
...
def get_top_movers() -> list:
    """
    Fetches top gainers and top losers SEPARATELY and merges them, so both
    LONG (off a big drop) and SHORT (off a big pump) candidates get a fair
    shot at making the list - instead of one abs()-sorted ranking that can
    end up skewed toward pumps.
    """
    try:
        resp = requests.get(
        ...
        gainers.sort(key=lambda x: x["change_pct"], reverse=True)   # biggest pumps first
        losers.sort(key=lambda x: x["change_pct"])                  # biggest drops first

        half = TOP_N // 2
        merged = gainers[:half] + losers[:TOP_N - half]
        # if one side didn't have enough candidates, top up from the other side
        if len(merged) < TOP_N:
            extra_needed = TOP_N - len(merged)
            extra_pool = gainers[half:] + losers[TOP_N - half:]
            merged += extra_pool[:extra_needed]

        merged.sort(key=lambda x: abs(x["change_pct"]), reverse=True)
        return merged[:TOP_N]
    except Exception as e:
...
def analyze_with_claude(symbol, price, change_pct, candles_1h, candles_4h) -> dict:
    ...
    prompt = (
        ...
        f"מחיר נוכחי {price}$ שינוי של {change_pct}% ב-24 שעות האחרונות.\n\n"
...
async def run_scan(bot: Bot):
    ...
    if not movers:
        ...
        return
    await bot.send_message(chat_id=CHAT_ID,
        text=f"📊 נמצאו *{len(movers)}* מטבעות — מנתח...", parse_mode="Markdown")
    results = []
    for coin in movers:
        symbol = coin["symbol"]
    ...
    for i, r in enumerate(results, 1):
        ...
        msg = (f"*{i}. {r['symbol']}* {emoji}\n"
               f"שינוי 24ש: {sign}{r['change_pct']}% | ביטחון: *{r['confidence']}%*\n"
...
async def cmd_top(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not movers:
        ...
        return
    lines = ["🔥 *Top Movers (עליות + ירידות)*\n"]
    for i, c in enumerate(movers[:15], 1):
        sign = "+" if c["change_pct"] >= 0 else ""
        lines.append(f"{i}. {c['symbol']} {sign}{c['change_pct']}%")
...
def main():
    ...
    if WEBHOOK_URL:
        logger.info(f"🚀 Scanner Bot starting in WEBHOOK mode on port {PORT}")
        app.run_webhook(
            listen="0.0.0.0",
            port=PORT,
        ...
        logger.warning("RENDER_EXTERNAL_URL not set - falling back to POLLING "
                        "(fine for local testing, but can cause Conflict errors "
                        "on Render if more than one instance ends up running)")
